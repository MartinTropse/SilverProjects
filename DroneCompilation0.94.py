...
print("Is this devil harddrive? y/n")
droneType = input()
...
if droneType == "y":
    for dirpath, subpath, filenames in os.walk(baseStr):
        if ".xlsx" in str(filenames):
            ...
            katgL.append("Excelfil Saknas")
        mp4pos = 0
        decFiles=[filenames[i][:-4] for i in range(0,len(filenames))]
        for x in set(decFiles):
            val=decFiles.count(x)
            if val > 1:
                StoreList.append(x)
                decFiles.remove(x)
                jpgHitList2 = []
                ...
                mFile=newPath+"/"+x+".MP4"
                pFile=newPath+"/"+x+".JPG"
                process = subprocess.Popen([exe,pFile], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
                ...
                for z in jpgList:
                    if z:
                        jpgHitList2.append(z.group(0))
                        #print(jpgHitList2)
                process2 = subprocess.Popen([exe, mFile], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
                mpList = [re.search(r'(File Modification Date/Time|Media Duration).+', output) for output in process2.stdout if type(output) == str] 
                for x in mpList:
                    if x:
                         mp4HitList2.append(x.group(0))                                
                         #print(mp4HitList2)        
                pathList.append(mFile)
                df = pd.DataFrame(columns=mp4col)
                df.loc[movPos] = jpgHitList2+mp4HitList2+pathList+Drone+katgL+diarL+comL
                movPos += 1
            ...
            if file[:-4] not in StoreList:
                ...
                if '.JPG' in path+file:
                     ...
                     while count == 0:
                         ...
                         for file in filenames:
                             if '.MP4' in path+file and file[:-4] not in StoreList:                                             
                                 mp4HitList = []
                                 process = subprocess.Popen([exe, path+"/"+file], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
